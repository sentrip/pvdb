...
#define PVDB_ALLOCATOR_MAX_LEVELS           8u
    ...
    uint                                initial_offset)
{
    ...
    uint size = 0;
    uint offset = initial_offset, impl_offset = 0;
    for (uint l = 0; l < n_levels; ++l) {
        const uint specific_offset = (levels[l].data_offset == -1u) ? offset : levels[l].data_offset;
        pvdb_allocator_init_level(alloc, l, n_levels, levels[l].max_allocations, levels[l].block_size, specific_offset, size, impl_offset);
    ...
    PVDB_ARRAY_IN(uint, max_allocations, PVDB_ALLOCATOR_MAX_LEVELS))
{
    ...
    for (uint i = 0; i < n_levels; ++i)
        mask_size += (1u + ((max_allocations[i] - 1u) / 32u));
    return n_levels * 5u + mask_size;
}
